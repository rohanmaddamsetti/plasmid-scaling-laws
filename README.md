...
Then, run concatenate-and-filter-chromosome-GhostKOALA-results.sh to generate
../results/chromosome_GhostKOALA_concatenated_ko_results.tsv, and 
successful_chromosome_GhostKOALA_concatenated_ko_results.tsv, which only contains rows that were
successfully mapped to a KEGG KO ID.  

Then run:  
TODO: FIGURE OUT WHAT TO WRITE FOR THIS NEXT LINE!!!
python remove-chromosomes-from-plasmid-Ghost-KOALA-results.py > ../results/filtered_successful_plasmid_GhostKOALA_concatenated_ko_results.tsv  
python get_unique_KEGG_IDs.py > ../results/chromosome.tsv  
...
Then, run:  
python get-chromosome-metabolic-KOs.py > ../results/chromosome_proteins_in_KEGG_metabolism.tsv  

IMPORTANT NOTE: get-chromosome-metabolic-KOs.py passes over genomes in which there is
a plasmid that is apparently larger than the chromosome. I found several genomes in
these complete genomes from RefSeq in which a 'plasmid' is actually the chromosome,
and the chromosome is most likely a plasmid. This is a small number of cases,
but this has a disproportionate effect on examining metabolic genes on plasmids.  

To generate a table of all KEGG metabolic genes in the chromosomes.  
