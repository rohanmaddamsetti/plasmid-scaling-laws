...
## Running GhostKOALA on a small set of 100 chromosomes for comparison.
...
GhostKOALA requires a FASTA list of up to 500,000 sequences, we choose an arbitrary set of 100 genomes.

IMPORTANT NOTE: make-100-chromosomes-protein-FASTA-db.py  

python make-100-chromosome-protein-FASTA-db.py  
mkdir ../results/chromosome-GhostKOALA-input-files/  
python make-chromosome-GhostKOALA-input-files.py  

Then, submit each of the input files in ../results/chromosome-GhostKOALA-input-files/ to the
GhostKOALA webserver at https://www.kegg.jp/ghostkoala.  

Save the output files generated by GhostKOALA in ../results/chromosome-GhostKOALA-output-files/,
with names like "chromosome_GhostKOALA_batch10_ko_results.tsv".  
