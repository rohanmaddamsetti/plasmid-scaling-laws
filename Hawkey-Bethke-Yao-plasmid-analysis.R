...
Hawkey.Bethke.Yao.data <- plasmid.copy.number.data %>%
    mutate(Reference="Hawkey2022") %>%
    full_join(YouLab.PCN.data)

## scatterplot of log10(Plasmid copy number) vs. log10(Plasmid length).
HBY.PCN.plot <- ggplot(Hawkey.Bethke.Yao.data,
    ...
    geom_smooth(
        ...
        formula=y~x)
## save the plot.
ggsave("../results/HBY-PCN.pdf",
       HBY.PCN.plot,height=5.75,width=5.75)
...
HBY.lm.model <- lm(
    ...
    data=Hawkey.Bethke.Yao.data)
## look at the linear regression.
summary(HBY.lm.model)

################################################################################
## Let's play with Zhengqing's his notion of plasmid capacity.
plasmid.copy.number.data2 <- plasmid.copy.number.data %>%
...
geom.mean.plasmid.copy.number.data <- plasmid.copy.number.data %>%
...
geomean.plasmid.copy.number.plot <- ggplot(geom.mean.plasmid.copy.number.data,
    ...
    ## add the linear regression.
    geom_smooth(
        data=geom.mean.plasmid.copy.number.data,
        inherit.aes=FALSE,
        method='lm',
        aes(x=log10(geomean_replicon_length),y=log10(geomean_CopyNumber)),
        color="light blue",
...
testplot2 <- ggplotRegression(
    geom.mean.plasmid.copy.number.data,
...
test3 <- plasmid.copy.number.data %>%
...
test4 <- plasmid.copy.number.data %>%
    mutate(total.plasmid.bp = replicon_length*CopyNumber) %>%
    group_by(AnnotationAccession) %>%
    summarize(
...
breseq.plasmid.copy.number.data <- read.csv("../results/Hawkey2022-breseq-copy-number.csv") %>%
    filter(SeqType == "plasmid")

## let's first plot the trend with these data.
## scatterplot of log10(Plasmid copy number) vs. log10(Plasmid length).
Hawkey.breseq.PCN.plot <- ggplot(breseq.plasmid.copy.number.data,
                                 aes(x=log10(replicon_length),
                                     y=log10(CopyNumber))) +
    geom_point() +
    theme_classic() +
    ...
    ## add the linear regression.
    geom_smooth(
        data=breseq.plasmid.copy.number.data,
        inherit.aes=FALSE,
        method='lm',
        aes(x=log10(replicon_length),y=log10(CopyNumber)),
        color="light blue",
...
breseq.plasmid.comp.data <- breseq.plasmid.copy.number.data %>%
    rename(BreseqCopyNumber = CopyNumber) %>%
    select(AnnotationAccession,SeqID,SeqType,BreseqCopyNumber)

benchmarking.PCN.estimate.data <- plasmid.copy.number.data %>%
...
benchmarking.scatterplot <- ggplot(
    ...
    geom_smooth(
        ...
        formula=y~x)
## save the plot
ggsave("../results/Hawkey2022-kallisto-breseq-benchmarking.pdf",
       benchmarking.scatterplot,height=4.75,width=4.75)

## make a linear regression model:
## log10(kallisto plasmid copy number) vs. log10(Breseq plasmid copy number).
## R-squared is 0.92.
benchmarking.lm.model <- lm(
    formula=log10(CopyNumber)~log10(BreseqCopyNumber),
    data=benchmarking.PCN.estimate.data)
## look at the linear regression.
